...
Bounded Python and Bun runs (20-second deadlines, upstream and bypass policies, disposable work/config/data directories) each exited 1 in about two seconds with native `Authentication required` stderr, empty stdout/raw, working output callbacks and restored instruction files. The selected native status was unauthenticated. No login/account/config change was attempted.
