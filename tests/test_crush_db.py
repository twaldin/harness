...
def _add_session(path, *, session_id, parent_session_id, prompt_tokens, completion_tokens, cost, updated_at):
    conn = sqlite3.connect(path)
    conn.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (session_id, parent_session_id, prompt_tokens, completion_tokens, cost, updated_at),
    )
    conn.commit()
    conn.close()
...
def test_crush_reads_latest_root_session(tmp_path):
    ...
    assert got == (42, 7, 0.001)


def test_crush_returns_none_when_db_missing(tmp_path):
    workdir = tmp_path / "repo"
    workdir.mkdir()
    assert _read_crush_session_totals(workdir, {"CRUSH_DATA_DIR": str(tmp_path / "missing")}) == (None, None, None)
