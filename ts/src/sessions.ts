...
//
// `openhands` on `rpc` is the sixth: direct HTTP + WebSocket against a
...
type SessionHarness = 'pi' | 'omp' | 'opencode' | 'claude-code' | 'amp' | 'openhands'
...
export interface SessionSpec {
  ...
  workdir: string
  /** Required; `rpc` for `pi`, `opencode` and `openhands`, `sdk` for `omp`, `claude-code` and `amp`. */
...
const SESSION_BACKENDS: Readonly<Record<SessionHarness, SessionBackend>> = { pi: 'rpc', omp: 'sdk', opencode: 'rpc', 'claude-code': 'sdk', amp: 'sdk', openhands: 'rpc' }

function isSessionHarness(name: string): name is SessionHarness {
  return name === 'pi' || name === 'omp' || name === 'opencode' || name === 'claude-code' || name === 'amp' || name === 'openhands'
}
...
function requireSessionHarness(name: unknown, backend: unknown): { harness: SessionHarness; backend: SessionBackend } {
  ...
  if (!isSessionHarness(resolved)) {
    throw new HarnessError(`Harness "${name}" has no live session backend; only "pi" (rpc), "opencode" (rpc), "openhands" (rpc), "omp" (sdk), "claude-code" (sdk) and "amp" (sdk) are supported`, 'unsupported-backend')
  }
