...
| `defaultModel` | used when RunSpec.model is unset; `amp`, `auggie`, `hermes`, `goose`, `copilot`, `cursor`, `mini-swe-agent` and `mistral-vibe` have none (empty sentinel), so upstream selection applies and the reported model is null |
...
```
["aider", "amp", "auggie", "claude-code", "cline", "codex", "continue-cli", "copilot", "crush", "cursor", "factory-droid", "gemini", "goose", "hermes", "kilo", "mini-swe-agent", "mistral-vibe", "omp", "openclaude", "opencode", "pi", "qwen", "swe-agent"]
```
