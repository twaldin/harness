...
`harness` is a dual-language (Python + TypeScript) adapter library that wraps fourteen
AI coding-agent CLIs — `claude-code`, `openclaude`, `opencode`, `codex`, `gemini`,
`aider`, `swe-agent`, `qwen`, `continue-cli`, `pi`, `factory-droid`, `kilo`, `crush`, `hermes` —
...
│   └── adapters/                14 adapter modules, one per CLI
...
│   ├── src/util.ts
│   └── src/adapters/            14 adapter modules, one per CLI
...
- Adapters: fourteen registered names, exact strings — `aider`, `claude-code`,
