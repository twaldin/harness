...
`harness` is a dual-language (Python + TypeScript) adapter library that wraps twenty
AI coding-agent CLIs — `claude-code`, `openclaude`, `opencode`, `codex`, `gemini`,
`aider`, `swe-agent`, `qwen`, `continue-cli`, `pi`, `omp`, `factory-droid`, `kilo`, `crush`, `hermes`, `goose`, `cline`, `copilot`, `cursor`, `mistral-vibe` —
...
│   └── adapters/                20 adapter modules, one per CLI
...
│   ├── src/util.ts
│   └── src/adapters/            20 adapter modules, one per CLI
...
- Adapters: twenty registered names, exact strings — `aider`, `claude-code`,
...
Current manifests do not meet that alignment: Python is `0.3.11` and
TypeScript is `0.2.15`. This is recorded skew, not a new release policy.
