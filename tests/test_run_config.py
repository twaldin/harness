...
@pytest.mark.parametrize("name", [n for n in list_adapters() if n not in ("claude-code", "codex", "hermes")])
...
@pytest.mark.parametrize("name", [n for n in list_adapters() if n not in ("claude-code", "aider", "continue-cli")])
