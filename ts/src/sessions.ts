...
/** `rpc` drives `pi` (and `opencode` over HTTP); `sdk` drives `omp` through the owned Bun bridge and `amp` through the Node SDK worker. Any other pairing (and `cli`) is `unsupported-backend`. */
export type SessionBackend = 'rpc' | 'sdk'

/** The harnesses with a live session backend. */
type SessionHarness = 'pi' | 'omp' | 'opencode' | 'amp'
...
export interface SessionSpec {
  ...
  workdir: string
  /** Required; `rpc` for `pi` and `opencode`, `sdk` for `omp` and `amp`. */
  ...
  model?: string
  /** Layered over the inherited process env; never mutated. For `omp`, entries conflicting with the owned `PI_CODING_AGENT_DIR` / `PI_CONFIG_DIR` are rejected; for `amp`, `AMP_SKIP_UPDATE_CHECK` may only be `"1"` and `AMP_URL` selects the endpoint. Must be empty on `opencode`. */
  env?: Record<string, string>
  /** Replaces the `pi` binary (rpc), the `bun` binary running the OMP bridge worker or the `node` binary running the Amp SDK worker: a bare name resolved on PATH or an absolute path. Rejected on `opencode`. */
  executable?: string
...
const SESSION_BACKENDS: Readonly<Record<SessionHarness, SessionBackend>> = { pi: 'rpc', omp: 'sdk', opencode: 'rpc', amp: 'sdk' }

function isSessionHarness(name: string): name is SessionHarness {
  return name === 'pi' || name === 'omp' || name === 'opencode' || name === 'amp'
}
...
function requireSessionHarness(name: unknown, backend: unknown): { harness: SessionHarness; backend: SessionBackend } {
  ...
  if (!isSessionHarness(adapter.name)) {
    throw new HarnessError(`Harness "${name}" has no live session backend; only "pi" (rpc), "opencode" (rpc), "omp" (sdk) and "amp" (sdk) are supported`, 'unsupported-backend')
  }
