...
well as source under Bun. OMP SDK cases use a synthetic package through the
real Bun worker, not an installed OMP/provider. Factory cases use the real
pinned SDKs with a synthetic CLI peer, not an installed Droid/provider.
...
Pi RPC, the OMP SDK bridge, native Factory Droid SDKs and caller-owned OpenCode
HTTP/SSE are implemented; other protocols/SDKs need qualification.
