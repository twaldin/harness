...
from typing import Awaitable, Callable, Coroutine, Literal, Sequence
...
_SESSION_BACKENDS: dict[str, Backend] = {"pi": "rpc", "omp": "sdk", "factory-droid": "sdk", "opencode": "rpc"}
...
class SessionSpec:
    ...
    `harness`         — "pi" (backend "rpc"), "omp" / "factory-droid" (backend
                        "sdk") or "opencode" (backend "rpc"); other registered
    ...
    `backend`         — "rpc" with "pi" / "opencode" or "sdk" with "omp" /
                        "factory-droid"; any other pairing and "cli" raise
                        `unsupported-backend`.
    `model`           — Pi: passed as `--model <model>`; OMP: handed to the
                        bridge (trimmed); Factory: native model ID sent as
                        `modelId` (trimmed; rejected with `resume`, which
                        restores the saved model); OpenCode: `provider/model`,
                        split at the first slash into the native provider and
                        model IDs (both non-empty). None keeps the agent's own
    ...
    `env`             — additions layered over the inherited environment. The
                        ...
                        entries are `invalid-options`. Factory requires a
                        non-empty `FACTORY_API_KEY` in the effective
                        environment (`launch-failed` before spawn otherwise)
                        and owns the SDK attribution variables. OpenCode
                        spawns nothing and rejects a non-empty env.
    `executable`      — bare binary name or absolute path; default "pi" for
                        rpc, "bun" for omp sdk, "droid" for factory-droid sdk.
                        Rejected for OpenCode.
    ...
    `resume`          — existing session to continue. Pi / OMP require
                        ...
                        `factory_droid.autonomy` must be unset. OpenCode
                        requires `session_file=None`, the same `endpoint` /
                        `workdir` and a full `ses_` ID, verified with
                        `GET /session/{id}` before anything else.
...
class SessionEvent:
    ...
    the Factory notification object (or the request / response envelope), or
    the OpenCode SSE event (`{"id", "type", "properties"}`)."""

    backend: Literal["rpc", "sdk"]
    harness: Literal["pi", "omp", "factory-droid", "opencode"]
    session_id: str
    turn_id: str | None
    request_id: str | None
    type: str
...
class SessionTurnResult:
    ...
    (omp sdk) the failing `sdk_settled` bridge frame or (factory-droid) the
    complete native `agent_turn_completed` notification. Usage inside it is
    the agent's own accounting exactly as sent (Factory: `tokenUsage` is this
    turn, `cumulativeTokenUsage` the session; `factoryCredits` are never
    money); nothing is aggregated or normalized.
...
def _validate_session_backend(name: str, backend: object) -> None:
    ...
    if backend == "cli":
        raise HarnessError("backend 'cli' has no live session support; use 'rpc' with harness 'pi' / 'opencode' or 'sdk' with harness 'omp' / 'factory-droid'", code="unsupported-backend")
    expected = _SESSION_BACKENDS.get(name)
    if expected is None:
        raise HarnessError(f"harness {name!r} has no live session support; only 'pi' (rpc), 'opencode' (rpc), 'omp' (sdk) and 'factory-droid' (sdk) are qualified", code="unsupported-backend")
    if backend != expected:
...
class LiveSession(abc.ABC):
    ...
    async def respond_approval(self, request_id: str, response: OpenCodeApprovalResponse) -> None:
        ...
        Factory sessions answer natively through `FactoryDroidOptions`
        callbacks instead."""
...
class _OwnedChild:
    ...
    async def dispose(self) -> _Disposal:
        ...
        while True:
            ...
            if not group_exists and self.exited.is_set() and not readers:
                break
            if now >= drain_end:
                break
...
class _ProcessSession(LiveSession):
    ...
    def __init__(self, spec: SessionSpec, prepared: PreparedCommand) -> None:
        ...
        self._child = _OwnedChild("omp sdk bridge" if self._sdk else "pi", self._loop, self._stderr, self._record_exit)
    ...
    async def _teardown(self) -> None:
        ...
        disposal = await self._child.dispose()
        for task in list(self._tasks):
            if task.done():
                continue
            try:
...
async def open_session(spec: SessionSpec) -> LiveSession:
    ...
    rpc`, `bun` running the OMP SDK bridge, or `droid exec` driven by the
    Factory SDK) and complete the identity handshake, or connect to the
    caller's OpenCode server (health, directory, session identity,
