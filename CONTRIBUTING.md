...
`node tests/node-opencode.mjs` and `node tests/node-droid-sdk.mjs` from `ts/`
to check packaged subprocess, RPC, SDK and HTTP/SSE behavior under Node as
well as source under Bun. OMP uses the real Bun worker with a synthetic SDK;
Amp uses the real Node worker with a synthetic SDK and finite native CLI
fixture; neither requires an installed provider or account. Claude and Factory
cases use the pinned real SDK packages from development dependencies plus
finite synthetic CLIs, never bundled CLIs or providers. Install Python
development dependencies with `uv sync --extra dev` before running these cases.
...
Pi RPC, the OMP/Amp SDK bridges, native Claude and Factory Droid SDK sessions
and caller-owned OpenCode HTTP/SSE are implemented; other protocols/SDKs need qualification.
