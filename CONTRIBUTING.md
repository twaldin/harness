...
`node tests/node-claude-sdk.mjs` and `node tests/node-opencode.mjs` from `ts/` to
check packaged subprocess, RPC, optional SDK and HTTP/SSE behavior under Node
as well as source under Bun. OMP cases use a synthetic package through the real
Bun worker. Claude cases use the pinned real SDK packages from development
dependencies plus a finite synthetic CLI, never a bundled CLI or provider.
Install Python development dependencies with `uv sync --extra dev` before
running these cases. No Windows lifecycle support is claimed.
...
Pi RPC, OMP/Claude SDK sessions and caller-owned OpenCode HTTP/SSE are implemented;
