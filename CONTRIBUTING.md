...
`node tests/node-amp-sdk.mjs`, `node tests/node-opencode.mjs` and
`node tests/node-openhands.mjs` from `ts/` to check packaged subprocess, RPC,
optional SDK-bridge and HTTP/SSE/WebSocket behavior under Node as well as
source under Bun. OMP uses the real Bun worker with a synthetic SDK; Amp uses
the real Node worker with a synthetic SDK and finite native CLI fixture.
Neither requires an installed provider or account. No Windows lifecycle
support is claimed.
...
Pi RPC, the OMP/Amp SDK bridges, caller-owned OpenCode HTTP/SSE and OpenHands
