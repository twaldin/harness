...
`harness` is a dual-language (Python + TypeScript) adapter library that wraps nineteen
AI coding-agent CLIs — `claude-code`, `openclaude`, `opencode`, `codex`, `gemini`,
`aider`, `swe-agent`, `qwen`, `continue-cli`, `pi`, `omp`, `factory-droid`, `kilo`, `crush`, `hermes`, `goose`, `cline`, `copilot`, `mistral-vibe` —
...
│   └── adapters/                19 adapter modules, one per CLI
...
│   ├── src/util.ts
│   └── src/adapters/            19 adapter modules, one per CLI
...
- Adapters: nineteen registered names, exact strings — `aider`, `claude-code`,
  `cline`, `codex`, `continue-cli`, `copilot`, `crush`, `factory-droid`, `gemini`, `goose`, `hermes`, `kilo`,
  `mistral-vibe`, `omp`, `openclaude`, `opencode`, `pi`, `qwen`, `swe-agent`. Lookup is case-sensitive.
...
Current manifests do not meet that alignment: Python is `0.3.10` and
TypeScript is `0.2.14`. This is recorded skew, not a new release policy.
