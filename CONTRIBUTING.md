...
`node tests/node-cline-sdk.mjs`, `node tests/node-opencode.mjs` and
`node tests/node-openhands.mjs` from `ts/` to check packaged subprocess, RPC,
optional SDK-bridge/SDK and HTTP/SSE/WebSocket behavior under Node as well as
...
neither requires an installed provider or account. Claude cases use the pinned
real SDK packages from development dependencies plus a finite synthetic CLI,
never a bundled CLI or provider. Install Python development dependencies with
`uv sync --extra dev`
