...
`harness` is a dual-language (Python + TypeScript) adapter library that wraps twenty-one
AI coding-agent CLIs — `claude-code`, `openclaude`, `opencode`, `codex`, `gemini`,
`aider`, `amp`, `swe-agent`, `qwen`, `continue-cli`, `pi`, `omp`, `factory-droid`, `kilo`, `crush`, `hermes`, `goose`, `cline`, `copilot`, `cursor`, `mistral-vibe` —
...
│   └── adapters/                21 adapter modules, one per CLI
...
│   ├── src/util.ts
│   └── src/adapters/            21 adapter modules, one per CLI
...
- Adapters: twenty-one registered names, exact strings — `aider`, `amp`, `claude-code`,
