...
@pytest.mark.parametrize("name", [n for n in list_adapters() if n not in ("claude-code", "codex", "copilot", "cursor", "hermes", "omp", "cline", "goose", "mistral-vibe", "mini-swe-agent", "kimi-code", "qoder")])
...
@pytest.mark.parametrize("name", [n for n in list_adapters() if n not in ("claude-code", "aider", "continue-cli", "omp", "amp", "mini-swe-agent")])
