...
`harness` is a dual-language (Python + TypeScript) adapter library that wraps twenty-five
AI coding-agent CLIs — `claude-code`, `openclaude`, `opencode`, `codex`, `gemini`,
`aider`, `amp`, `auggie`, `swe-agent`, `mini-swe-agent`, `qwen`, `continue-cli`, `pi`, `omp`, `factory-droid`, `kilo`, `crush`, `hermes`, `goose`, `cline`, `copilot`, `cursor`, `mistral-vibe`, `kimi-code`, `kiro` —
...
│   └── adapters/                25 adapter modules, one per CLI
...
│   ├── src/util.ts
│   └── src/adapters/            25 adapter modules, one per CLI
...
- Adapters: twenty-five registered names, exact strings — `aider`, `amp`, `auggie`, `claude-code`,
