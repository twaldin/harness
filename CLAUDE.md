...
`aider`, `amp`, `swe-agent`, `qwen`, `continue-cli`, `pi`, `omp`, `factory-droid`, `kilo`, `crush`, `hermes`, `goose`, `cline`, `copilot`, `cursor`, `mistral-vibe`, `kiro` —
...
- Adapters: twenty-two registered names, exact strings — `aider`, `amp`, `claude-code`,
...
Current manifests do not meet that alignment: Python is `0.3.12` and
TypeScript is `0.2.16`. This is recorded skew, not a new release policy.
